...
{
	private Node234Tree root = new Node234Tree();
	private double noComparisions = 0;
	
	public void insert(int key)
	{
		Node234Tree node = root;
		while (true)
		{
			if (node.isFull()) {
				split(node);
	...
	private void split(Node234Tree node)
	{
		Integer itemC = node.removeItem();
		Integer itemB = node.removeItem();
		Node234Tree parent;
		Node234Tree child2 = node.disconnect(2);
		Node234Tree child3 = node.disconnect(3);
		Node234Tree newNode = new Node234Tree();
		
		if (node == root) {
			root = new Node234Tree();
			parent = root;
			root.connect(0, node);
		}
		else {
		...
		{
			Node234Tree temp = parent.disconnect(j);
	...
	public Node234Tree find(Integer key)
	{
		Node234Tree node = root;
		while (true)
		{
	...
	private void recDisplayTree(Node234Tree thisNode, int level, int childNumber)
	{
		...
		{
			Node234Tree nextNode = thisNode.getChild(j);
			if (nextNode != null) {
	...
	private Integer getMin(Node234Tree node)
	{
		if (node == null) {
			return null;
		}
	...
	private Integer getMax(Node234Tree node)
	{
		if (node == null) {
			return null;
		}
	...
	private Integer getSuccessor(Node234Tree node, int key)
	{
		if (node == null) {
	...
	private Integer getPredecessor(Node234Tree node, int key)
	{
		if (node == null) {
			throw new NoSuchElementException();
		}
	...
	public Node234Tree getNextChild(Node234Tree node, int key)
	{
		int j=0;
